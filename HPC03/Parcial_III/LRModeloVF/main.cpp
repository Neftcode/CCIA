...
int main(int argc, char* argv[]){
    ...
    Eigen::VectorXd thetaOut_train; // coeficientes
    std::vector<float> costo_train;
    Eigen::VectorXd thetaOut_test; // coeficientes
    std::vector<float> costo_test;

    /* Se desempaqueta la tupla obtenida del objeto de la clase
     * Regresión Lineal */
    std::tuple<Eigen::VectorXd, std::vector<float>> salidaGD_train = RL.descentGradient(X_train, y_train, theta, alpha, iterations);
    std::tuple<Eigen::VectorXd, std::vector<float>> salidaGD_test = RL.descentGradient(X_test, y_test, theta, alpha, iterations);
    std::tie(thetaOut_train, costo_train) = salidaGD_train;
    std::tie(thetaOut_test, costo_test) = salidaGD_test;
    ...
    extraction.vectorToFile(costo_train, "vectorCosto_train.txt");
    extraction.matrixToFile(thetaOut_train, "vectorTheta_train.txt");
    extraction.vectorToFile(costo_test, "vectorCosto_test.txt");
    extraction.matrixToFile(thetaOut_test, "vectorTheta_test.txt");
    ...
    Eigen::MatrixXd y_train_hat = (X_train*thetaOut_train*devFeatures).array()+muFeatures;
    Eigen::MatrixXd y_test_hat = (X_test*thetaOut_test*devFeatures).array()+muFeatures;
    Eigen::MatrixXd y1 = df.col(8).topRows(13600);
    Eigen::MatrixXd y2 = df.col(8).bottomRows(3400);

    /* A continuación se determina que tan bueno es nuestro
     * modelo utilizando la métrica R2 */

    float metricaR2_train = RL.R2Score(y1, y_train_hat);
    std::cout << metricaR2_train << std::endl;
    float metricaR2_test = RL.R2Score(y2, y_test_hat);
    std::cout << metricaR2_test << std::endl;

    extraction.matrixToFile(y_train_hat, "prediccion_train.txt");
    extraction.matrixToFile(y_test_hat, "prediccion_test.txt");
