...
/* Se requiere crear una aplicación que lea ficheros que contengan set de datos
   (dataSet) en CSV debe presentar una clase que represente la extracción de los datos
   entre la carga de los datos, la normalización de los datos y la manipulación
   de los datos con Eigen: ExToEigen.

   Adicional se requiere otra clase que presente el cálculo de la regresión lineal */
...
int main(int argc, char *argv[])
{
    ...
    /* Se crean dos vectores para prueba y entrenamiento respectivamente en unos para
       probar el modelo de regresión lineal */
    ...
    auto muData = extraction.Promedio(matrizDF);
    auto muFeatures = muData(0, 11);
    auto muEscalado = matrizDF.rowwise()-matrizDF.colwise().mean();
    auto sigmaData = extraction.DesviacionEstandar(muEscalado);
    auto sigmaFeatures = sigmaData(0, 11);
    Eigen::MatrixXd y_train_hat = (X_train*thetaOut*sigmaFeatures).array()+muFeatures;
